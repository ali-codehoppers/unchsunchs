...
public class SigninSheetGenerator : GenericGenerator
{
    ...
    protected override void GenerateDocument(Document doc, DocumentType docType, PdfWriter writer)
    {
        ...
        for (int i = 0; i < columnsHeadeing.Length; i++)
        {
            table.AddCell(new Phrase(columnsHeadeing[i], FontFactory.GetFont("Arial", 5)));
        }

        //Add the remaining (Data Rows) to the table
        PdfPCell cellSerialNo, cellHotWorks = null;

        for (int j = 0; j < dataRows; j++)
        {
            cellSerialNo = new PdfPCell(new Phrase((j + 1).ToString(), FontFactory.GetFont("Arial", 6)));
            cellHotWorks = new PdfPCell(new Phrase("YES/NO", FontFactory.GetFont("Arial", 6)));
